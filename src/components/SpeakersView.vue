<template>
  ...
  <div class="max-sm:h-[392px] max-sm:bg-center max-sm:relative
              bg-no-repeat bg-cover bg-[url('http://localhost:81/images/speakers/bg.png')]
              w-screen h-screen">
    ...
    <div class="flex flex-col sm:flex-row gap-20">
      ...
      <div class="flex flex-col gap-2 w-full sm:w-1/3">
        ...
        <div class="flex items-center gap-2">
          <input type="checkbox" id="black" v-model="filters.color" value="Black" class="accent-white" />
          <label for="black" class="text-white">Black</label>
        </div>
        <div class="flex items-center gap-2">
        ...
        <div class="flex items-center gap-2">
          <input type="checkbox" id="blackAndSteel" v-model="filters.color" value="Black and Steel" class="accent-white" />
          <label for="blackAndSteel" class="text-white">Black and Steel</label>
        </div>
        <div class="flex items-center gap-2">
          <input type="checkbox" id="cream" v-model="filters.color" value="Cream" class="accent-white" />
          <label for="cream" class="text-white">Cream</label>
        </div>
        <div class="flex items-center gap-2">
          <input type="checkbox" id="brown" v-model="filters.color" value="Brown" class="accent-white" />
  ...
  <div v-else class="bg-[#383434] text-white grid grid-cols-3 gap-4 p-4">
    <div v-for="speaker in filteredSpeakers" :key="speaker.id" class="speaker-item cursor-pointer bg-gray-700 rounded-lg p-4" @click="goToItem(speaker)">
      <img :src="'http://localhost:81/images/' + speaker.image" :alt="speaker.name" class="w-full h-auto rounded-md" />
      <div class="flex flex-col gap-4 p-4">
        <h2 class="text-l font-bold">{{ speaker.name }}</h2>
        <p class="text-l font-bold">Price: {{ speaker.price }}</p>
      </div>
    </div>
  </div>
  <Footer></Footer>
...
export default {
  ...
  data() {
    return {
      ...
      colors: ["Black", "Cream", "Brown", "Black and Brass", "Black and Steel"]
    };
  },
  mounted() {
  ...
  computed: {
    filteredSpeakers() {
      return this.speakers.filter(speaker => {
        const priceMatch =
            parseFloat(speaker.price.replace('$', '').replace(',', '')) >= this.filters.minPrice &&
            parseFloat(speaker.price.replace('$', '').replace(',', '')) <= this.filters.maxPrice;

        const colorMatch = this.filters.color.length === 0 || this.filters.color.includes(speaker.color);
  ...
  methods: {
    async fetchSpeakersData() {
      try {
        const response = await fetch('http://localhost:81/speakers.php');
        if (response.ok) {
    ...
    goToItem(item) {
      ...
      this.$router.push({path: `/speakers/item/${item.id}`, params: { item }})
    }
  },
};
</script>
