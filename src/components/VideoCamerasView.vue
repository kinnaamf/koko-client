<template>
  ...
  <div class="max-sm:h-[392px] max-sm:bg-center max-sm:relative
              ...
              w-screen h-screen">
    <h2 class="text-white uppercase text-7xl font-black absolute top-1/2 left-10">Action cameras</h2>
  </div>
  ...
  <div v-show="showFilters" class="bg-[#383434] text-white p-4 transition-all duration-300 ease-in-out">
    <div class="flex flex-col sm:flex-row gap-20">
      ...
      <div class="flex flex-col gap-2 w-full sm:w-1/3">
        ...
        <div class="flex items-center gap-2">
          <input type="checkbox" id="white" v-model="filters.color" value="White" class="accent-white" />
          <label for="white" class="text-white">White</label>
        </div>
      </div>
    </div>
  </div>
  ...
  <div v-else class="bg-[#383434] text-white grid grid-cols-3 gap-4 p-4">
    <div v-for="action in filteredActions" :key="action.id" class="action-item cursor-pointer bg-[#2e2e2e] rounded-lg p-4" @click="goToItem(action)">
      <img :src="'http://localhost:81/images/' + action.image" :alt="action.name" class="w-full h-auto rounded-md" />
      <div class="flex flex-col gap-4 p-4">
        <h2 class="text-l font-bold">{{ action.name }}</h2>
        <p class="text-l font-bold">Price: {{ action.price }}</p>
      </div>
    </div>
  </div>
  <Footer></Footer>
...
export default {
  ...
  computed: {
    filteredActions() {
      return this.actions.filter(action => {
        const priceMatch =
            parseFloat(action.price.replace('$', '').replace(',', '')) >= this.filters.minPrice &&
            parseFloat(action.price.replace('$', '').replace(',', '')) <= this.filters.maxPrice;

        const colorMatch = this.filters.color.length === 0 || this.filters.color.includes(action.color);

        const typeMatch = this.filters.type.length === 0 || this.filters.type.includes(action.type);
  ...
  methods: {
    async fetchActionData() {
      try {
        const response = await fetch('http://localhost:81/action.php');
        if (response.ok) {
