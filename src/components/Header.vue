      ...
      <div class="flex gap-5 text-l max-sm:text-sm">
        <div>
          <a class="self-end uppercase text-l max-sm:text-sm max-sm:hidden" href="http://localhost:81/login.php"
             style="color: #71717A">{{ username || 'ACCOUNT' }}</a>
        </div>
        <LangSwitcher/>
      </div>
    </div>
        ...
        <div class="flex gap-5 max-lg:gap-2 items-center">
          ...
          <div
            ...
            <div class="flex gap-3 max-lg:gap-3">
              ...
              <label for="">
                <input v-model="showItems"
                       class="border-0 border-none focus:border-none focus:outline-none font-medium inp w-full"
                ...
                <ul class="grid grid-cols-6 gap-10 justify-between">
                  <li v-for="item in filteredItems" :key="item.id" class=" w-[150px]">
                    <a :href="getItemUrl(item)">
                      <img :src="'http://localhost:81/images/' + item.image" alt=item.name class="w-[150px] h-[150px]">
...
import items from "@/images/items.json"
...
export default {
  ...
  methods: {
    ...
    async fetchItems(category = null) {
      try {
        const response = await fetch(
            category
                ? `http://localhost:81/get_category.php?category=${category}`
                : `http://localhost:81/get_category.php`
        )
    ...
    getImagePath(item) {
      ...
      switch (item.category) {
        case 'Amps':
          basePath = 'http://localhost:81/images/amps';
          break;
        case 'Cameras':
          basePath = 'http://localhost:81/images/cameras';
          break;
        case 'Headphones':
          basePath = 'http://localhost:81/images/headphones';
          break;
        case 'Speakers':
          basePath = 'http://localhost:81/images/speakers';
          break;
        case 'Videocameras':
          basePath = 'http://localhost:81/images/video';
          break;
        default:
          basePath = 'http://localhost:81/images/';
      }
